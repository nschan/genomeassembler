...
```{r prepare quast, message = F, warning = F}
...
```{r quast page, message = F, warning = F}
#| eval: !expr params$quast
#| results: asis
...
```{r busco data, message = F, warning = F}
...
```{r busco page, message = F, warning = F}
#| eval: !expr params$busco
#| results: asis
#
...
```{r merqury add plots and valueboxes, message = F, warning = F}
#| eval: !expr params$merqury
#| results: asis
...
#| echo: false
#| output: asis
versions <- yaml::read_yaml("versions.yml")
lapply(1:length(versions), \(process) {
  proc  <-  versions[[process]] |> str_split(":\\\n ") |> flatten()
  proc_name  <-  proc[[1]][1]
  tools  <-  proc[[2]][1] |>
    str_split("\\\n ") |>
    lapply(\(x) trimws(x)) |>
    flatten()
  tools_df <- lapply(1:length(tools), \(toolnum) {
    tool_info  <- tools[[toolnum]][1] |>
      str_split(": ")  %>%
      flatten()
    tool_name = tool_info[[1]]
    tool_version = tool_info[[2]]
