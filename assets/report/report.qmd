...
paste0('```{r}\n
            ...
            facet_wrap(read_type~stat, scales = "free_y", ncol=2) +
            fill_scale_plots +
...
for (i in 1:length(unique(fastplong_reports$group))) {
    cat(paste0('## ', unique(fastplong_reports$group)[i], '\n\n'),
        ...
        paste0(fastplong_reports %>%
            ...
            filter(sample == unique(fastplong_reports$group)[i]) %$%
            sum(value) %>%
    ...
    lapply(\(x) {
            ...
            mutate(
                sample = str_extract(x %>% basename(),
                                ".+?(?=_\\.assembly|\\-assemble|links|longstitch|ragtag|medaka|pilon|\\-run|\\-polish)"),
                stage = case_when(
                    str_detect(x, "_ragtag") ~ "RagTag",
                    str_detect(x, "_medaka") ~ "medaka",
                    str_detect(x, "_pilon") ~ "pilon",
